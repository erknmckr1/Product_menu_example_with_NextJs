          ...
          <div className="w-full h-full pt-5">
            <div className="w-auto h-full flex gap-x-3 items-center justify-center">
              {categories.map((item,index)=>(
                <button onClick={()=>setClickedCategory(item)} key={index} className={` ${clickedCategory === item ? "outline px-7 py-2 border border-1 hover:outline ease-in duration-100" :"px-7 py-2 border border-1 hover:outline ease-in duration-100"}`}>
                {item}
              </button>
              ))}
            </div>
                    ...
                    <div className="w-2/3 h-full p-5">
                      <div className="w-full h-full flex flex-col">
                        ...
                        <div className="flex justify-between items-center border-b-2 border-black">
                          <span className="text-red-400 text-[20px] sm:text-[25px]">{item.title}</span>
